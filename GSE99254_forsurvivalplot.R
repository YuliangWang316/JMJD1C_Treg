...
Tregtumor <- FindNeighbors(Tregtumor, dims = 1:20,k.param = 20,n.trees = 100,reduction = "pca",annoy.metric = "euclidean")
Tregtumor <- FindClusters(Tregtumor, resolution = 0.9,algorithm = 1,modularity.fxn = 1 )
Tregtumor.marker<-FindAllMarkers(Tregtumor,only.pos = TRUE)
DotPlot(Tregtumor, features = "JMJD1C") + RotatedAxis()
VlnPlot(Tregtumor,features = "JMJD1C",sort = TRUE,pt.size = 0)

library(ggplot2)
library(ggpubr)
library(RColorBrewer)
Tregtumor<-RunLDA(Tregtumor,labels = Tregtumor$seurat_clusters)
Tregtumor<-RunUMAP(Tregtumor,reduction = "lda",reduction.name = "lda_umap",dims = 1:5)
Tregtumor<-RunTSNE(Tregtumor,reduction = "lda",reduction.name = "lda_tsne",dims = 1:5)
...
new.cluster.ids <- c("lo","hi","lo","lo","hi","lo","lo")
names(new.cluster.ids) <- levels(Tregtumor)
Tregtumor <- RenameIdents(Tregtumor, new.cluster.ids)
DimPlot(Tregtumor, reduction = "lda_tsne", label = TRUE, pt.size = 0.5) + NoLegend()
...
Tregpbmc<-subset(pbmc,idents = "CD4_C8-FOXP3")
remove(Tregtumor)
...
write.table(hilomarkers,"c:/Users/xjmik/Desktop/GSE99254hilomarkers.txt",sep = "\t")
remove(hilomarkers)
...
write.table(markers1,file = "c:/Users/xjmik/Desktop/GSE99254hivsother.txt",sep = "\t")
remove(name)
remove(markers1)
