...
remove(all.genes)
new.cluster.ids <- c("lo","lo","hi","hi","lo")
...
write.table(hilomarkers,"c:/Users/xjmik/Desktop/GSE98638hilomarkers.txt",sep = "\t")
remove(hilomarkers)
...
write.table(markers1,file = "c:/Users/xjmik/Desktop/GSE98638hivsother.txt",sep = "\t")
remove(name)
remove(markers1)
a<-read.table("c:/Users/xjmik/Desktop/1C_signature.txt",sep = "\t")
a<-a[,1]
pbmc_raw<-AddModuleScore(pbmc_raw,features = a,name = "J1C",random.seed = 1)
remove(a)
JMJD1C<-select(pbmc_raw@meta.data,starts_with("J1C"))
pbmc_raw$JMJD1C_signature<-apply(JMJD1C,MARGIN = 1,median)
remove(JMJD1C)
Idents(pbmc_raw)<-pbmc_raw$majorCluster
FeaturePlot(pbmc_raw,features = "JMJD1C_signature" ,label = TRUE)
